...
static Handle<Value> RavenTitanium_getBinding(const Arguments& args)
{
	HandleScope scope;

	if (args.Length() == 0) {
		return ThrowException(Exception::Error(String::New("RavenTitanium.getBinding requires 1 argument: binding")));
	}
	...
	titanium::bindings::BindEntry *extBinding = ::RavenTitaniumBindings::lookupGeneratedInit(
...
static void RavenTitanium_init(Handle<Object> exports)
{
	HandleScope scope;
	...
	exports->Set(String::New("getBinding"), FunctionTemplate::New(RavenTitanium_getBinding)->GetFunction());
}

static void RavenTitanium_dispose()
{
	HandleScope scope;
	if (bindingCache.IsEmpty()) {
		return;
	...
	for (uint32_t i = 0; i < length; ++i) {
		...
		titanium::bindings::BindEntry *extBinding =
			::RavenTitaniumBindings::lookupGeneratedInit(*binding, bindingLength);
...
static titanium::bindings::BindEntry RavenTitaniumBinding = {
	"ti.raven",
	RavenTitanium_init,
	RavenTitanium_dispose
};

// Main module entry point
extern "C" JNIEXPORT void JNICALL
Java_ti_raven_RavenTitaniumBootstrap_nativeBootstrap
	(JNIEnv *env, jobject self)
{
	titanium::KrollBindings::addExternalBinding("ti.raven", &RavenTitaniumBinding);
	titanium::KrollBindings::addExternalLookup(&(::RavenTitaniumBindings::lookupGeneratedInit));
}
